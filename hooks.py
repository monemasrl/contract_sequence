...
def pre_init_hook(cr):
    ...
    is launched.
    """
    cr.execute("ALTER TABLE contract_contract ADD COLUMN code character varying;")
    cr.execute("UPDATE contract_contract SET code = id;")
...
def post_init_hook(cr, registry):
    ...
    for contract_id in contracts.ids:
        cr.execute(
            "UPDATE contract_contract SET code = %s WHERE id = %s;",
            (
